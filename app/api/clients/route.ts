...
export async function GET() {
  ...
  const [rows, petRows] = await Promise.all([getDb().select().from(clients).where(eq(clients.organizationId, manager.organizationId)).orderBy(asc(clients.name)), getDb().select().from(pets).where(eq(pets.organizationId, manager.organizationId))]);
  return Response.json({ clients: rows.map(client => ({ ...client, pets: petRows.filter(pet => pet.clientId === client.id) })) });
}
...
export async function POST(request: Request) {
  ...
  if (!manager) return Response.json({ error: "Acesso não autorizado" }, { status: 403 });
  try {
    const body = await request.json() as { name?: string; phone?: string; notes?: string; pet?: string; breed?: string; size?: string };
    ...
    const petName = body.pet?.trim();
    const createdPets = petName ? await getDb().insert(pets).values({ organizationId: manager.organizationId, clientId: rows[0].id, name: petName, breed: body.breed?.trim() || null, size: body.size?.trim() || "Não informado", notes: body.notes?.trim() || null }).returning() : [];
    return Response.json({ client: { ...rows[0], pets: createdPets } }, { status: 201 });
  } catch (error) {
