...
export async function PATCH(request: Request) {
  ...
  if (!manager) return Response.json({ error: "Acesso não autorizado" }, { status: 403 });
  try {
    const data = await request.json() as { id?: number; paidCents?: number; paymentMethod?: string; courtesy?: boolean; status?: "confirmed" | "cancelled"; oldName?: string; oldPhone?: string; name?: string; phone?: string };
    if (data.id && data.status) { await getDb().update(appointments).set({ status: data.status }).where(and(eq(appointments.id, data.id), eq(appointments.organizationId, manager.organizationId))); return Response.json({ updated: true }); }
