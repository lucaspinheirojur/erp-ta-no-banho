...
type AnalyticsMovement = { id:string; date:string; description:string; category:string; type:"Entrada"|"Saída"; method:string; value:number };
...
export default function Home() {
  ...
  const analyticsRevenue=totalReceived+packageReceived;
  const analyticsBalance=analyticsRevenue-totalExpenses;
  const analyticsPending=appointments.reduce((sum,item)=>sum+(item.payment?0:(item.price??0)),0)+packagePending;
  const paidRecords=appointments.filter(item=>item.payment).length+packageUsage.filter(item=>item.payment).length;
  const averageTicketValue=paidRecords?analyticsRevenue/paidRecords:0;
  const servicePerformance=services.map(service=>{const related=appointments.filter(item=>item.service===service.name);return{name:service.name,count:related.length,revenue:related.reduce((sum,item)=>sum+(item.payment?.amount??0),0)};}).sort((a,b)=>b.revenue-a.revenue||b.count-a.count).slice(0,5);
  const maxServiceRevenue=Math.max(1,...servicePerformance.map(item=>item.revenue));
  const appointmentStatus=[
    {label:"Confirmados",value:appointments.filter(item=>item.status==="confirmado").length,color:"#12a3ad"},
    {label:"Aguardando",value:appointments.filter(item=>item.status==="aguardando").length,color:"#ed62aa"},
    {label:"Concluídos",value:appointments.filter(item=>item.status==="concluido").length,color:"#22a681"},
    {label:"Bloqueios",value:appointments.filter(item=>item.status==="bloqueado").length,color:"#9aa9ac"},
  ];
  const totalStatus=Math.max(1,appointmentStatus.reduce((sum,item)=>sum+item.value,0));
  const statusGradient=appointmentStatus.reduce((result,item,index)=>{const before=appointmentStatus.slice(0,index).reduce((sum,current)=>sum+current.value,0)/totalStatus*100;const after=before+item.value/totalStatus*100;return `${result}${index?", ":""}${item.color} ${before}% ${after}%`;},"");
  const paymentMethods=["PIX","Dinheiro","Cartão de débito","Cartão de crédito"].map(method=>({method,value:[...appointments,...packageUsage].filter(item=>item.payment?.method===method).reduce((sum,item)=>sum+(item.payment?.amount??0),0)}));
  const maxPaymentMethod=Math.max(1,...paymentMethods.map(item=>item.value));
  const paymentColors=["#0798a3","#ed62aa","#73c8c5","#a7b6b8"];
  const paymentTotal=Math.max(1,paymentMethods.reduce((sum,item)=>sum+item.value,0));
  const paymentGradient=paymentMethods.reduce((result,item,index)=>{const before=paymentMethods.slice(0,index).reduce((sum,current)=>sum+current.value,0)/paymentTotal*100;const after=before+item.value/paymentTotal*100;return `${result}${index?", ":""}${paymentColors[index]} ${before}% ${after}%`;},"");
  const evolutionMonths=Array.from({length:7},(_,index)=>{const date=new Date();date.setDate(1);date.setMonth(date.getMonth()-(6-index));const key=`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}`;const value=appointments.filter(item=>item.payment&&item.date?.startsWith(key)).reduce((sum,item)=>sum+(item.payment?.amount??0),0)+packageUsage.filter(item=>item.payment&&item.startDate?.startsWith(key)).reduce((sum,item)=>sum+(item.payment?.amount??0),0);return{key,label:new Intl.DateTimeFormat("pt-BR",{month:"short",year:"2-digit"}).format(date).replace(" de ","/"),value};});
  const maxEvolution=Math.max(1,...evolutionMonths.map(item=>item.value));
  const analyticsMovements:AnalyticsMovement[]=[
    ...appointments.filter(item=>item.payment).map(item=>({id:`appointment-${item.id}`,date:item.date??todayIso,description:`${item.service} · ${item.pet} / ${item.client}`,category:"Atendimento",type:"Entrada" as const,method:item.payment?.method??"Não informado",value:item.payment?.amount??0})),
    ...packageUsage.filter(item=>item.payment).map(item=>({id:`package-${item.id}`,date:item.startDate??todayIso,description:`Pacote · ${item.pet} / ${item.client}`,category:"Pacote",type:"Entrada" as const,method:item.payment?.method??"Não informado",value:item.payment?.amount??0})),
    ...expenses.map(item=>({id:`expense-${item.id}`,date:item.expenseDate,description:item.description,category:item.category,type:"Saída" as const,method:item.paymentMethod,value:-(item.amountCents/100)})),
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "analises" && (
          <div className="content analytics-page lane-analytics">
            <section className="analytics-titlebar"><div><p>INTELIGÊNCIA DO NEGÓCIO</p><h1>Saúde financeira</h1><h2>Visão consolidada dos resultados do Tá no Banho.</h2></div><div className="analytics-actions">{reportMode==="month"?<label><span>Período</span><input type="month" value={reportMonth} onChange={event=>setReportMonth(event.target.value)}/></label>:<><label><span>De</span><input type="date" value={reportFrom} onChange={event=>setReportFrom(event.target.value)}/></label><label><span>Até</span><input type="date" value={reportTo} min={reportFrom} onChange={event=>setReportTo(event.target.value)}/></label></>}<button onClick={()=>setReportMode(reportMode==="month"?"custom":"month")}>{reportMode==="month"?"Período personalizado":"Selecionar mês"}</button><a href={`/api/finance/report?${reportQuery}&format=pdf`} download>▧ Baixar PDF</a><a href={`/api/finance/report?${reportQuery}&format=xls`} download>▦ Baixar Excel</a></div></section>
            <section className="lane-kpis" aria-label="Indicadores financeiros"><article className="received"><span className="lane-kpi-icon"><DashboardIcon name="revenue"/></span><div><small>Recebido</small><strong>{formatCurrency(analyticsRevenue)}</strong><p>Pagamentos confirmados</p></div></article><article className="pending"><span className="lane-kpi-icon"><DashboardIcon name="return"/></span><div><small>A receber</small><strong>{formatCurrency(analyticsPending)}</strong><p>Atendimentos e pacotes pendentes</p></div></article><article className="expenses"><span className="lane-kpi-icon">↓</span><div><small>Despesas</small><strong>{formatCurrency(totalExpenses)}</strong><p>Saídas registradas</p></div></article><article className="result"><span className="lane-kpi-icon"><DashboardIcon name="appointments"/></span><div><small>Resultado</small><strong>{formatCurrency(analyticsBalance)}</strong><p>Saldo operacional</p></div></article></section>
            <section className="lane-charts">
              <article className="panel lane-chart-card evolution-card"><h3>Evolução do faturamento</h3><div className="evolution-chart"><div className="evolution-grid"><span/><span/><span/><span/></div><div className="evolution-bars">{evolutionMonths.map(item=><div key={item.key}><strong>{item.value?formatCurrency(item.value):"R$ 0"}</strong><i style={{height:`${Math.max(5,item.value/maxEvolution*100)}%`}}/><small>{item.label}</small></div>)}</div></div></article>
              <article className="panel lane-chart-card payment-method-card"><h3>Formas de pagamento</h3><div className="lane-payment-visual"><div className="lane-payment-donut" style={{background:`conic-gradient(${paymentGradient})`}}><span>{formatCurrency(analyticsRevenue)}</span></div><div className="lane-payment-list">{paymentMethods.map((item,index)=><div key={item.method}><i className={`method-${index}`}/><span>{item.method}</span><strong>{formatCurrency(item.value)}</strong></div>)}</div></div></article>
              <article className="panel lane-chart-card revenue-service-card"><h3>Receitas por serviço</h3><div className="vertical-services">{servicePerformance.length?servicePerformance.slice(0,4).map(item=><div key={item.name}><strong>{formatCurrency(item.revenue)}</strong><span><i style={{height:`${Math.max(5,item.revenue/maxServiceRevenue*100)}%`}}/></span><small>{item.name}</small></div>):<p>Os serviços aparecerão após os primeiros recebimentos.</p>}</div></article>
            </section>
            <section className="panel movements-panel"><div className="movements-head"><div><small>MOVIMENTAÇÃO FINANCEIRA</small><h3>Últimas movimentações</h3></div><button onClick={()=>goTo("financeiro")}>Ver financeiro completo →</button></div><div className="movements-table"><div className="movement-row movement-header"><span>Data</span><span>Descrição</span><span>Categoria</span><span>Tipo</span><span>Forma de pagamento</span><span>Valor</span></div>{analyticsMovements.length?analyticsMovements.map(item=><div className="movement-row" key={item.id}><span>{new Date(`${item.date}T12:00:00`).toLocaleDateString("pt-BR")}</span><strong>{item.description}</strong><span>{item.category}</span><span className={item.type==="Entrada"?"movement-in":"movement-out"}>{item.type==="Entrada"?"↓":"↑"} {item.type}</span><span>{item.method}</span><strong className={item.value>=0?"movement-value-in":"movement-value-out"}>{formatCurrency(item.value)}</strong></div>):<div className="analytics-empty">As movimentações aparecerão após o primeiro pagamento ou despesa.</div>}</div></section>
          </div>
        )}
      </section>
