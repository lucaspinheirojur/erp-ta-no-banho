      ...
      <section className="workspace">
        ...
        {view === "analises" && (
          <div className="content analytics-page">
            ...
            <section className="panel analytics-toolbar"><div><small>PERÍODO DO RELATÓRIO</small><strong>{reportMode==="month"?"Visão mensal":"Período personalizado"}</strong></div><div className="report-mode"><button className={reportMode==="month"?"active":""} onClick={()=>setReportMode("month")}>Mês</button><button className={reportMode==="custom"?"active":""} onClick={()=>setReportMode("custom")}>Personalizado</button></div>{reportMode==="month"?<label>Mês<input type="month" value={reportMonth} onChange={event=>setReportMonth(event.target.value)}/></label>:<><label>De<input type="date" value={reportFrom} onChange={event=>setReportFrom(event.target.value)}/></label><label>Até<input type="date" value={reportTo} min={reportFrom} onChange={event=>setReportTo(event.target.value)}/></label></>}</section>
            <section className="analytics-kpis" aria-label="Indicadores analíticos"><article><span>Receita total</span><strong>{formatCurrency(analyticsRevenue)}</strong><small>Atendimentos e pacotes recebidos</small></article><article><span>Saldo operacional</span><strong>{formatCurrency(analyticsBalance)}</strong><small>Receitas descontadas das despesas</small></article><article><span>Ticket médio</span><strong>{formatCurrency(averageTicketValue)}</strong><small>Por recebimento confirmado</small></article><article><span>Taxa de retorno</span><strong>{returnRate}%</strong><small>{clients.length} clientes ativos</small></article></section>
            <section className="analytics-grid">
              <article className="panel analytics-card analytics-wide"><div className="analytics-card-head"><div><small>DESEMPENHO COMERCIAL</small><h3>Receita por serviço</h3></div><span>{servicePerformance.reduce((sum,item)=>sum+item.count,0)} agendamentos</span></div><div className="service-bars">{servicePerformance.length?servicePerformance.map(item=><div className="service-bar-row" key={item.name}><div><b>{item.name}</b><small>{item.count} atendimentos</small></div><span><i style={{width:`${Math.max(4,item.revenue/maxServiceRevenue*100)}%`}}/></span><strong>{formatCurrency(item.revenue)}</strong></div>):<div className="analytics-empty">Os serviços aparecerão aqui após os primeiros agendamentos.</div>}</div></article>
              <article className="panel analytics-card"><div className="analytics-card-head"><div><small>OPERAÇÃO</small><h3>Status da agenda</h3></div><span>{appointments.length} registros</span></div><div className="status-visual"><div className="status-donut" style={{background:`conic-gradient(${statusGradient})`}}><span><strong>{appointments.length}</strong><small>Total</small></span></div><div className="status-legend">{appointmentStatus.map(item=><div key={item.label}><i style={{background:item.color}}/><span>{item.label}</span><strong>{item.value}</strong></div>)}</div></div></article>
              <article className="panel analytics-card"><div className="analytics-card-head"><div><small>RECEBIMENTOS</small><h3>Formas de pagamento</h3></div><span>{formatCurrency(analyticsRevenue)}</span></div><div className="payment-bars">{paymentMethods.map(item=><div key={item.method}><label><span>{item.method}</span><strong>{formatCurrency(item.value)}</strong></label><i><b style={{width:`${item.value/maxPaymentMethod*100}%`}}/></i></div>)}</div></article>
              <article className="panel analytics-card analytics-highlight"><div><small>EFICIÊNCIA FINANCEIRA</small><h3>Margem operacional</h3><strong>{analyticsRevenue?Math.round(analyticsBalance/analyticsRevenue*100):0}%</strong><p>{formatCurrency(totalExpenses)} em despesas sobre {formatCurrency(analyticsRevenue)} de receita.</p></div><div className="margin-gauge"><i style={{width:`${Math.max(0,Math.min(100,analyticsRevenue?analyticsBalance/analyticsRevenue*100:0))}%`}}/></div></article>
            </section>
            <section className="panel report-panel"><div className="panel-head"><div><small>CENTRAL DE RELATÓRIOS</small><h3>Exportar dados financeiros</h3></div><span className="payment-help">PDF para conferência · Excel para análise</span></div><div className="report-controls"><div className="report-downloads"><a href={`/api/finance/report?${reportQuery}&format=pdf`} download>↓ Baixar PDF</a><a href={`/api/finance/report?${reportQuery}&format=xls`} download>↓ Baixar Excel</a></div></div></section>
          </div>
        )}
      </section>
